	...
	function expect($cmd, $cases_groups, $answers_groups, $outfile, &$debug){
				...
				for($i=0; !$answered && ($i<count($answers)); $i++)
				{
					if ($answers[$i][0] == $casename)
					{
						...
						{ 
							...
							// Answer
							if (isset($case[3]) && $case[3]=="finish" || isset($custom_answer[3]) && $custom_answer[3]=="finish")
							{
								$finished = true; // Mark as finished to avoid triggering a error if EOF is generated by answer write!
							}
		...
		switch($casename){
			case EXP_EOF:
				if ($finished)
				{
					$result = AUTOMATA_FINISHED; // Normal EOF after finish action
				}	
				else
				{
					$result = AUTOMATA_EOF; // Unhandled EOF result
				}
				break;
			case EXP_TIMEOUT:
